...
4) CRM
  ...
  Файл SubscriberCrudTest содержит 4 позитивных теста по CRUD операциям с абонентами. Каждый тест помечен тегом sequence, то есть выполняются они в последовательности. Если тест завалился, следующий за ним не пойдет (нам нечего будет редактировать, если не удастся создать абонента).
...
Тесты на API CRM написаны на основании swagger-спецификации (приведена в проекте), все ручки собраны в postman коллекции (также прикреплена к проекту).
...
Автотесты написаны на основании ручных тест-кейсов на e2e сценарии

Автотесты лежат в папке src/test/java/com/Nexign/e2e - два файла: для тарифа "Классика" и "Помесячный" соответственно.

1. Креды для подключения к базам данных Postgres лежат в переменных окружения
2. 
